...
bool DataBank::reconnect()
{
    ...
    if(mysql_real_connect(&myCont, host.c_str(), user.c_str(), password.c_str(), table.c_str(), port, NULL, 0))
    {
        ...
        LOG_DEBUG("mysql connect succeed");
    }else{
        mysql_close(&myCont);
        LOG_DEBUG("mysql failed");
    };
#else
    LOG_DEBUG("undefined MY_SQL");
#endif
    return isconnected;
}
...
void DataBank::close()
{
    if(isConnected())
    {
        ...
        mysql_close(&myCont);
        #endif
        LOG_DEBUG("mysql close ok!");
    }else{
        LOG_DEBUG("mysql close error: is no open");
    }
}
...
bool DataBank::apply(const char* sql)
{
    if(isConnected())
    {
        ...
        if(ret != 0)
        {
            LOG_WARN("mysql apply error ret=%d",ret);
        }
        return ret == 0;
    }
    LOG_WARN("mysql apply error: is no open");
    return false;
}
...
bool DataBank::find(DataQuery& query, const char* sql)
{
    ...
    if(apply(sql))
    {
        ...
        if(result)
        {
            ...
            LOG_WARN("mysql find result error");
            return false;
        }
    }
    #endif
