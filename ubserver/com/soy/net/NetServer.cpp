...
bool NetServer::listener(int port, int maxfd)
{
    ...
    {
        LOG_DEBUG("open server ok port=%d serid=%d", port, m_listen_fd);
        this->m_maxfd = maxfd;
        this->m_isopen = true;
...
void NetServer::read_handler(NetNode* node, char* bytes, size_t size)
{
    node->LoadBytes(bytes, size);
    try{
        MsgHandler* packet = NULL;
        while((packet = node->NextMessage()))
        {
            on_read(node, packet);
        }
    }catch(...){
...
void NetServer::on_read(NetNode* node, MsgHandler* packet)
{
    RUN_MAIN(new SocketEvent(SOCKET_READ_DATA, this, node, packet));
}
