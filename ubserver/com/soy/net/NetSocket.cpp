...
static SOCKET_T net_connect(const char* host, int port)
{
    ...
    if ((listen_fd = socket(AF_INET, SOCK_STREAM, 0)) == -1)
    {
        ...
        {
            LOG_WARN("socket connect error: %s:%d", host, port);
        }else{
            LOG_DEBUG("socket connect ok: %s:%d:%d", host, port, listen_fd);
            return listen_fd;
        }
    }
    return -0;
...
void NetSocket::read_handler(char* bytes, size_t size)
{
    this->LoadBytes(bytes, size);
    try{
        MsgHandler* packet = NULL;
        while((packet = this->NextMessage()))
        {
            on_read(packet);
        }
    }catch(...){
...
void NetSocket::on_read(MsgHandler* message)
{
    RUN_MAIN(new SocketEvent(SOCKET_READ_DATA, this, this, message));
}
